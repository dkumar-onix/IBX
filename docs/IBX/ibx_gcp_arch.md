...
* **Pipeline Collisions & Lock Contention:** Monolithic Terraform state files and shared execution branches cause frequent state lock failures and Git merge conflicts when multiple teams request resources simultaneously.
...
| **Execution Engine** | Split across GitHub and GCP Cloud Build | Native GitHub Actions powered by Workload Identity Federation (WIF) | **Single Plane of Glass:** Consolidates logs, execution traces, and PR plan comments in one platform without long-lived keys. |
| **IaC State Management** | Centralized Terraform state / tfvars append loops | Terragrunt Monorepo with dynamic per-project backend derivation (`tfstate-<project_id>`) | **Blast Radius Containment:** Sub-second plans, isolated state locks, and zero cross-project state dependencies. |
| **Security Enforcement** | Manual review or post-apply audit | CI Pipeline Policy-as-Code (Checkov / OPA / TFLint) | **Preventative Governance:** Blocks unencrypted storage, public IPs, or missing VPC connectors at PR time. |
...
```text
infra-live/
├── root.hcl                 # Layer 0: Global backend & provider generation
├── _envcommon/              # Layer 3: Base module wiring
│   ├── gcs-bucket.hcl
│   ├── compute-vm.hcl
...
│   ├── secret-manager.hcl   #
│   └── load-balancer.hcl
├── dev/                     # Layer 1: Environment Scope
│   ├── env.hcl
│   ├── ibx-analytics-dev/   # GCP Service Project A
│   │   └── us-central1/     # Layer 2: Region Scope
│   │       ├── region.hcl
│   │       └── raw-landing-zone/  # Target Leaf Path
│   │           └── terragrunt.hcl # Generated by ServiceNow
...
No static service account keys are stored in GitHub. All authentication relies on OIDC short-lived tokens.
