...
> **Design goal:** keep the existing request path fully automated and responsive while the improved architecture is introduced incrementally. Independent requests can execute in parallel; requests targeting the same Terraform state are serialized.

```mermaid
%%{init: {"theme": "base", "themeVariables": {"fontSize": "20px", "nodePadding": "18px", "primaryTextColor": "#111827", "lineColor": "#4B5563"}, "flowchart": {"nodeSpacing": 60, "rankSpacing": 80, "curve": "basis"}}}%%
flowchart TD

    subgraph REQUEST[1. REQUEST & INTAKE]
        DEV[Developer / Service Account]
        SN[ServiceNow Request]
        API[Provisioning API<br/>Cloud Run / API Gateway]
        DEV --> SN --> API
    end

    subgraph ORCH[2. ASYNC ORCHESTRATION]
        Q[Pub/Sub<br/>Provisioning Queue]
        WF[Cloud Workflows<br/>Orchestration & State]
        REG[Provisioning Registry<br/>Resource → Module → Repo → Environment]
        API --> Q --> WF --> REG
    end

    subgraph CHANGE[3. GIT-BASED CHANGE]
        REPO[Target Project<br/>Infrastructure Repository]
        MOD[Central Terraform<br/>Module Repository]
        GH[GitHub<br/>Branch + Pull Request]
        REG --> REPO
        REG --> MOD
        REPO --> GH
        MOD -. Versioned Modules .-> REPO
    end

    subgraph CONTROL[4. AUTOMATED CONTROL GATES]
        CI[Reusable GitHub Actions]
        TEST[Validate / Test / Lint]
        SEC[Security Scanning]
        POL[Policy-as-Code]
        PLAN[Terraform / OpenTofu<br/>Plan + Saved Plan]
        GH --> CI
        CI --> TEST --> SEC --> POL --> PLAN
    end

    subgraph DEPLOY[5. APPROVAL & DEPLOYMENT]
        APPROVAL[Risk-Based Approval]
        APPLY[Apply Exact<br/>Approved Plan]
        GCP[GCP Infrastructure]
        PLAN --> APPROVAL --> APPLY --> GCP
    end

    subgraph CLOSE[6. VALIDATION & FEEDBACK]
        VALIDATE[Post-Provision<br/>Validation / Smoke Tests]
        STATE[Provisioning State Store]
        STATUS[ServiceNow<br/>Status + Evidence]
        GCP --> VALIDATE --> STATE --> STATUS
    end

    %% Cross-cutting control and feedback
    WF -. Request Correlation ID .-> STATE
    CI -. Execution Evidence .-> STATE
    STATUS -. Completion / Failure .-> SN
```

### Architecture intent

- **Existing requests continue to flow through automation** rather than waiting for the future-state architecture to be completed.
- **Pub/Sub + Workflows provide asynchronous processing**, allowing independent requests to progress concurrently.
- **GitHub remains the change-control boundary**, with reusable CI/CD enforcing consistent controls across project repositories.
- **Central Terraform modules remain reusable and versioned**; individual requests modify the target project configuration, not the module implementation.
- **Security, policy, testing and approval remain deterministic gates** before infrastructure changes reach GCP.
- **Agentic capabilities can be added around the flow** for request interpretation, configuration assistance, plan explanation and failure diagnosis without bypassing governance.
